...
final class ActionPickerPanelController {
    ...
    private var onChainSelected: ((ActionChain) -> Void)?
    ...
    func show(
        actions: [SpellAction],
        chains: [ActionChain] = [],
        actionsManager: ActionsManager? = nil,
        onSelectAction: @escaping (SpellAction) -> Void,
        onSelectChain: ((ActionChain) -> Void)? = nil,
        onDismiss: @escaping () -> Void
    ) {
        self.onActionSelected = onSelectAction
        self.onChainSelected = onSelectChain
        ...
        // Calculate height: each row is ~32px + 2px spacing, plus padding
        let rowHeight: CGFloat = 32
        let spacing: CGFloat = 2
        let verticalPadding: CGFloat = 12
        var totalItems = actions.count
        if !chains.isEmpty {
            totalItems += chains.count + 1 // +1 for the separator/header
        }
        let totalHeight = CGFloat(totalItems) * rowHeight + CGFloat(max(0, totalItems - 1)) * spacing + verticalPadding
    ...
    private func selectChain(_ chain: ActionChain) {
        removeEventMonitors()
        panel?.orderOut(nil)
        panel = nil
