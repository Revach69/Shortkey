...
struct MenuBarPopoverView: View {
    ...
    @State private var chainEditorMode: ChainEditorMode?
    ...
    enum ChainEditorMode: Identifiable {
        case add
        case edit(ActionChain)

        var id: String {
            switch self {
            case .add: return "add_chain"
            case .edit(let chain): return chain.id.uuidString
            }
        }
        ...
        var chain: ActionChain? {
            switch self {
            case .add: return nil
            case .edit(let chain): return chain
            }
        }
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ActionsListView(
                ...
                chainEditorMode: $chainEditorMode
            )
        ...
        .sheet(item: $chainEditorMode) { mode in
            ChainEditorSheet(
                chain: mode.chain,
                availableActions: actionsManager.actions,
                onSave: { chain in
                    if case .edit = mode {
                        actionsManager.updateChain(chain)
                    } else {
                        actionsManager.addChain(chain)
                    }
                    chainEditorMode = nil
                },
                onCancel: {
                ...
                onDelete: mode.chain != nil ? {
                    if let chain = mode.chain {
                        actionsManager.deleteChain(id: chain.id)
                    }
                    chainEditorMode = nil
                } : nil
            )
