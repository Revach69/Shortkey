...
```bash
cd /Users/drorl/Projects/Spellify/firebase
...
Edit `Spellify/Services/FirebaseBackendManager.swift`:
...
1. Open `Spellify.xcodeproj` in Xcode
2. Right-click on `Services` folder → "Add Files to Spellify"
3. Select:
   - `Spellify/Services/CryptoService.swift`
   - `Spellify/Services/FirebaseBackendManager.swift`
4. Make sure "Copy items if needed" is checked
5. Click "Add"

### 9. Update SpellifyApp.swift

Add device registration on app launch:

```swift
// In SpellifyApp.swift, after Firebase initialization:
Task {
